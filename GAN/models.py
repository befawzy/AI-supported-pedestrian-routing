...
def get_noise(shape, noise_type):
    '''
    generate noise for the generator decoder   
        ...
        noise_type: string
            noise type name, either gaussian or uniform


        Returns
        ----------
        tensor of randomly generated number, from a given probability distribution
...
class Encoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        Parameters
        ----------
        obs_traj: Tensor of shape (obs_len(default is 8), batch, 2)
...
class PoolHiddenNet(nn.Module):
    """Pooling module, as explained in the report"""
    def __init__(
...
class TrajectoryGenerator(nn.Module):
    ...
    def add_noise(self, _input, seq_start_end, user_noise=None):
        """
        feed noise to the generators decoder, using the above defined noise function. the function is
        implemented to generate the random noise but based on the already observed values (MomenTUM data), 
        i.e. to have values that are consistent with the observed values 
        ...
        relation between different types of noise and outputs, default None
